...
//If the searchBar doesn't contains the "active" class when clicked the classes of the search bar and icon will change
...
searchBar.onkeyup = () => {
  //The "keyup" event on "searchTerm" caputires the current value of the search input in the "searchTerm" variable.
  ...
  xhr.onload = () => {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        //The response from "search.php" is set as the inner HTML of "userList", updating the user with the search results.
  ...
  xhr.onload = () => {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        ...
        let data = xhr.response;
        if (!searchBar.classList.contains("active")) {
          //f not active, updates usersList's inner HTML with the response data, which is the updated user list.
          usersList.innerHTML = data;
        }
      }
    }
